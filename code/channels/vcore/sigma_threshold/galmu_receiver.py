...
def decode_secret(path, decoding_duration, sampling_frequency, vcore_threshold):
    ...
    while True:
        ...
        if avg[0] > vcore_threshold:
            print(
                f"Decoded bit: 1, AVG: {avg[0]:.2f}, Threshold: {vcore_threshold:.2f}."
            )
            bit = 1
            consecutive_zeros = 0
        else:
            print(
                f"Decoded bit: 0, AVG: {avg[0]:.2f}, Threshold: {vcore_threshold:.2f}."
            )
            bit = 0
            consecutive_zeros += 1
...
def main():
    ...
    # utils.look_for_handshake(path, vcore_threshold, handshake_frequency)
    ...
    secret_in_bits = decode_secret(
        path, decoding_duration, decoding_frequency, vcore_threshold
    )
